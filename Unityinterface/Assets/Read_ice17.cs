...
public class Read_ice17 : MonoBehaviour
{
    ...
    {
        map_row_string = File.ReadAllLines(Application.dataPath + "/supercell/ice17.data");
        ...
        for (int i = 0; i < map_Collections.Count; i++)//Z方向是长度就是容器的大小，也就是*读入数据*有多少有效的行
        {
            ...
            GameObject atomInstance = Instantiate(atom, container.transform);
        ...
        for (int i = 0; i < bond_Collections.Count; i++)
        {
            ...
            lineRenderer1.startWidth = 0.02f;
            lineRenderer1.endWidth = 0.02f;
