...
public class Read_768 : MonoBehaviour
{
    ...
    {
        map_row_string = File.ReadAllLines(Application.dataPath + "/hbond_fromServer/ice/Nanoice/768.lammpstrj");
        ...
        GameObject container1 = GameObject.Find("bond_container");                // 找到模型存放容器
        GameObject[] kids = new GameObject[container1.transform.childCount];

        int j = 0;
        foreach (Transform kid in container1.transform)
        {
        ...
        for (int i = 0; i < map_Collections.Count; i++)//Z方向是长度就是容器的大小，也就是*读入数据*有多少有效的行
        {
            ...
            GameObject atomInstance = Instantiate(atom, GameObject.Find("container").transform);
            //print("before bug?");
            atomInstance.transform.parent = container1.transform;
            //print("after bug?");
