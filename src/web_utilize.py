...
def load_data(candidate_filenames: str) -> Tuple[Any, Any, Any, Any, Any]:
    feature_inputs = torch.load(f'../calculated_results/{candidate_filenames}/feature_inputs.pt')
    targets = torch.load(f'../calculated_results/{candidate_filenames}/targets.pt')
    atom_coords = torch.load(f'../calculated_results/{candidate_filenames}/atom_coords.pt')
    atom_types = torch.load(f'../calculated_results/{candidate_filenames}/atom_types.pt')
    box_list = torch.load(f'../calculated_results/{candidate_filenames}/box_list.pt')
    return feature_inputs, targets, atom_coords, atom_types, box_list


# @st.cache_data
def load_model_and_process_data(sample_selection, model, device):
...
def display_rdf(atom_coords, atom_types, sample_selection, time_selection, max_distance, num_bins, device, rdf):
    st.subheader('Radical Distribution Function')
    ...
    with selection:
        ...
        atom1 = selected_sample_property[rdf_selection[0]]
        atom2 = selected_sample_property[rdf_selection[1]]
        fig.update_layout(title=f'the radical distribution function for '
                                f'({atom1}, {atom2})',
...
def main():
    st.title('Metaverse Chemistry Laboratory')
    # st.write("Reminder: do the preprocessing first, like sort_and_process_file('dump-8.reax.mgNO3', 'mgno3_8.lammpstrj')")
    if 'calculation_started' not in st.session_state:
        st.session_state['calculation_started'] = False
    with st.sidebar:
        st.subheader('Options')
        sample_selection = st.selectbox('Select Sample', ['NaCl', 'mgno3_8', 'CuCl2', 'FeCl3', 'Na2CO3', 'FB_reax'],
        ...
        if st.session_state['calculation_started']:
            ...
            feature_inputs, atom_coords, atom_types, I, J, K = load_model_and_process_data(sample_selection, model,
                                                                                           device)
            ...
            st.write(
                '0 for initial system, last for stable case. The initial 30% is input, while the subsequent 70% is predicted.')
            st.write('\n')
    ...
    if st.session_state['calculation_started']:
        ...
        display_rdf(atom_coords, atom_types, sample_selection, time_selection, max_distance, num_bins, device, rdf)
